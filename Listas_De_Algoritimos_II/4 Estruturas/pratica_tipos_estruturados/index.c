...
int valida_turma(Turma ** turmax){//função auxiliar
  if (turmas[0] == NULL){
...
Turma* cria_turma(char ID){//case 1
  int i;
  printf("\nCriando nova turma...");
...
void listar_turmas(Turma ** turmax){//case 2
  int i;
...
void matricular_aluno(Turma * turma, char * nome, int mat){//case 3
  int i, n;
  turma->vagas -= 1;
  for (i = 0; i < MAX_VAGAS; i++){
...
void lancar_notas(Turma * turmax){//case 4
  int i, n;
  float contMedia;
  for (i = 0; i < MAX_VAGAS; i++){
    contMedia = 0;
...
void imprime_turmas(Turma * turmax){//case 5
  int i;
...
void buscar_turmas(Turma ** turmax,char ID, int index){//case 6
  int i;
  for (i = 0; i < index; i++){
    if(turmax[i]->id == ID){
